...
from create_image_pool import create_image_pool

db = create_image_pool(r'E:\github\photomosaic\emoji')

INSIDE_OUT = 'inside-out'
TOP_DOWN = 'top-down'

def create_mosaic(source, output, pool_dir, 
                  ratio, method, repeat, threshold, verbose, imagepool=db):
        '''
        ...
        size_ratio = ratio
        #imagepool = connect_database(":memory:")
...
def inside_out(grid, output, tile_size, pool_dir, db, repeat, threshold,
    ...
    if not start:
        start_x = int(len(grid[0]) / 2)
        start_y = int(len(grid) / 2)
    else:
        start_x = start[0]
        start_y = start[1]
    end_x = int(start_x + 1)
...
def subdivide_source(image, tile_size):
    ...
    '''
    width = int(image.size[0] / tile_size[0])
    height = int(image.size[1] / tile_size[1])
